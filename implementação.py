...
def lista_magneticos_da_serie_completa():
    global tabelas, resolut, magnetico
    ...
    if len(magnetico) == 0:
        for tabela in range(0, len(tabelas)):
            ...
            try:
                for quali in range(0, len(html_qualidades)):
                    resolut.append(f"{html_num_epi[0].string} {html_qualidades[0].string}")

                for link_mag in range(len(html_magnetic)):
                    magnetico.append(str(html_magnetic[0]).split('"')[3])
            except:
                pass
...
def main():
    global resolut, magnetico, tabelas
    ...
    lista_magneticos_da_serie_completa()
    opcoes_de_uso()
